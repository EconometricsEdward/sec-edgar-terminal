...
The Portfolio history view loads 4, 8, or 12 calendar quarters ending at the selected report. Separate synchronized charts show concentration, disclosed position count, and total reported value. Each measure includes a change against the immediately preceding calendar quarter when both public reports are complete and comparable. Concentration changes use percentage points. Missing or limited-scope quarters never silently substitute an earlier comparison baseline. A selected holding adds report share, quantity, and value history, first observation within the loaded window, and repeated changes across comparable adjacent reports.

`GET /api/fund-13f/history?cik=…&period=…&keys=…` returns one compact quarter projection with at most 32 explicitly requested security keys. The history view requests only its selected security, and two browser workers load the quarters progressively. A bounded cache reuses compatible manager/quarter/security observations across window and holding changes. Previously loaded portfolio metrics stay visible while another holding's observations load. Full historical holdings tables are not retained in the browser. Failed quarters stay retryable; missing reports and incomplete evidence remain visible gaps. An unrequested position never becomes a zero. Amendments use the currently available public filing sequence for each quarter, not a reconstruction of what was known on quarter end.

History charts use zero-based axes fitted to each measure, separate units, keyboard quarter inspection, and exact-value/source tables. Hover previews a quarter; leaving the chart restores the deliberately selected quarter. Quick selections expose the largest positions, with search covering the full selected report. A CSV export includes exact quarter metrics, coverage and source information; unavailable observations remain empty. Manager, reporting-quarter, and security identities are checked again before data reaches the chart or company panel.

Quarter projections preserve their original retrieval and metadata-check dates. A stale fallback remains labeled as saved evidence and is not publicly cached as a freshly checked history response. Reported-value changes are not investment returns, cash flows, trading activity or changes in total assets under management.
