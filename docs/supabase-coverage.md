...
The existing Market overview was durably published during the production build: 1,498 companies across 11 sectors, preserving its original 2026-09-13T05:56:09.772Z source time. CFTC's existing four prepared heads remain in service.

At 08:28:57 UTC, PostgreSQL database size was 128,748,691 bytes and 6,206 private objects totaled 309,658,246 bytes. PostgreSQL database size excludes WAL and other system disk usage; these are observed data footprints, not month-to-date billing or visitor-capacity measurements. Three small indexes were subsequently added for the current, last-good, and rollback version references. The existing source index is retained; there is no destructive retention cleanup.

Live staged Compare checks preserved AAPL's annual metrics, periods, and source references while reducing its transferred response from 8,884 to 6,706 bytes (24.5%). A fresh browser rendered AAPL/JPM annual, quarterly, and TTM comparisons and expanded original filing and calculation evidence without site-origin console errors. Final activation verification adds representative non-pilot and alias reads.
