...
For a complete worked example, download the [prefilled 100-ticker CSV](https://secedgarterminal.com/portfolio/portfolio-demo-100.csv) or [prefilled 100-ticker XLSX](https://secedgarterminal.com/portfolio/portfolio-demo-100.xlsx), and [preview the captured research results](https://secedgarterminal.com/workspace/demo). The files contain a company research universe with no allocation assumptions. The results are a dated capture of actual public SEC evidence, preserving company reporting periods, retrieval times, source links, and incomplete coverage. They demonstrate the research workflow and are not an investment recommendation. A later refresh can change figures, filings, and coverage. Users can open a separate saved copy from the preview to explore the full workspace and refresh on request.
...
The UI exports a selected unique-issuer table to CSV, a five-sheet XLSX workbook, structured JSON, and a source-backed Markdown brief. “Copy research context” produces the same evidence brief with context for another research tool; it does not call an LLM. XLSX sheets are Holdings, Company research, Portfolio summary, Sources, and Coverage & methodology. Workbook cells are literal values, and CSV text is protected against spreadsheet formula injection.

The richer export schema is `edgar.portfolio.research.v1`, with `input_schema_version: "edgar.portfolio.v1"`, export generation time, research capture time, `positions`, `companies`, `allocation`, `coverage`, `sources`, `warnings`, `exclusions`, and `methodology`. It is an evidence package, not an import template. Reported public evidence, application calculations and user-entered positions remain distinct. Exported metrics retain units, reporting periods, source accessions/URLs and formula/input details. Notes are excluded by default with an explicit opt-in. Allocation values are included by default only under the visible export option; users can exclude allocations, quantities and holding dates to share company research alone.
