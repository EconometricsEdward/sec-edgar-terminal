...
The UI exports a selected unique-issuer table to CSV, a six-sheet XLSX workbook, structured JSON, and a source-backed Markdown brief. “Copy research context” produces the same evidence brief with context for another research tool; it does not call an LLM. XLSX sheets are Holdings, Company research, Portfolio summary, Sources, Coverage & methodology, and Analytics. Workbook cells are literal values, and CSV text is protected against spreadsheet formula injection.
