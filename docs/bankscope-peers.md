...
Compare has three entry points: Peer Explorer, CAMELS, and Selected banks. Peer Explorer and CAMELS automatically match a bank without needing its peers' individual Call Reports to have been prepared. Selected banks retains the four-bank chart/table workflow. The `panel` and `lens` URL parameters preserve the selected view; older links remain supported.
...
`lens=camels` is a shareable view under Compare → CAMELS. `category` preserves the selected lens. Six visual letter tabs open one component at a time, with three public indicators, peer distributions, and paired historical paths. There are no supervisory scores, composite grades or compliance conclusions. Management is represented only by operating proxies; sensitivity by maturity/securities proxies. Actual CAMELS ratings are confidential.

A sticky context strip names the selected legal bank, reporting quarter, group size, unweighted median basis and subject exclusion. Cyan always represents the selected bank; lavender identifies peers and their statistics. Peer Explorer pairs an interactive scatter map with a metric spotlight covering all 24 ratios. The map supports credit/capital, earnings and funding presets, configurable axes, individual peer inspection and direct bank comparison. Its crosshair medians use only peers reporting both axes, exclude the subject and duplicates, and require five valid peers; the plotted count is explicit. This joint reporting sample may differ from the single-metric distributions.

Both views use full-distribution histograms, bank/median markers, interquartile bands and larger paired history charts. Six quick metrics support rapid spotlight switching. Definitions, chart guides, regulatory scope, exact figures, source records and UBPR references are collapsed by default. Bank identities, ratio units, framework exclusions and reporting counts stay visible. The full group disclosure is accessible from either lens. Regulatory charts appear within the Capital lens, use neutral reference markers, and name the bank. Colors identify scope, never quality.
...
Both views include quarterly paths alongside current-quarter distributions. CAMELS changes can use the immediately preceding quarter or first retained period. Each chart compares bank and group on one metric-specific scale, keeps missing values as gaps, and labels both changes. Activating a history chart opens and focuses the shared history disclosure, which mounts its larger interactive chart on demand. Named comparison rows, adjacent-quarter and full-window changes, quarterly figures, historical capital framework, valid counts and per-period source records remain available in nested details. Tooltip coverage is specific to the hovered quarter.
