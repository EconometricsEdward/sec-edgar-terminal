...
export async function GET(request, { params }) {
  ...
  if (/^\d+$/.test(selected.ticker)) {
    ...
    if (selected.basis !== 'annual' || selected.end || selected.asOf) return Response.json({ error: 'Public X-17A-5 analysis supports the latest annual report. Quarterly, TTM and historical-cutoff figures are not inferred.', code: 'UNSUPPORTED_BROKER_DEALER_BASIS' }, { status: 422, headers: privateHeaders });
    try {
