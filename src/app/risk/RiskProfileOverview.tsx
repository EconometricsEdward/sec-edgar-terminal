  ...
  return <div className={s.profile}>
    <section className={s.overview} aria-labelledby="risk-profile-title">
      ...
      <aside className={s.balance} aria-labelledby="balance-structure-title">
        ...
        <div className={s.fundingComparison}><h3>{comparisonTitle}</h3><p>{bank ? 'Reported balances on a common dollar scale' : 'Separate reported balances on a common dollar scale'}</p>{comparisons.map(row => <div key={row.label} className={s.fundingRow}><div><span>{row.label}</span><strong>{formatRiskValue(row.item.value)}</strong></div><div className={s.fundingTrack}>{isNumber(row.item.value) && row.item.value >= 0 && <span style={{width:`${row.item.value/maxBalance*100}%`}}/>}</div></div>)}{view.lens.id === 'corporate' && <p className={s.caption}>Investments retain their reported scope and may include nonmarketable assets. Credit, rate and liquidity risk remain; noncurrent investments are separate from cash. {isNumber(balance.cashAndMarketableSecurities.value) && <>Cash and investments total {formatRiskValue(balance.cashAndMarketableSecurities.value)}.</>}</p>}</div>
