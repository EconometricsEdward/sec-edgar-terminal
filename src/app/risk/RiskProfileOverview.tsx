...
export default function RiskProfileOverview({ data, profile, onInspect, onExposures }: { data: RiskData; profile: RiskProfile; onInspect: (id: string, missing?: boolean) => void; onExposures?: () => void }) {
  ...
  const comparisons = bank ? [{ label:'Net loans', item:balance.loans }, { label:'Deposits', item:balance.deposits }] : [{ label:'Cash', item:balance.cash }, view.lens.id === 'corporate' ? { label:'Tagged debt', item:balance.debt } : { label:'Total liabilities', item:balance.liabilities }];
  const comparisonTitle = bank ? 'Net loans and deposits' : view.lens.id === 'corporate' ? 'Cash and tagged debt' : 'Cash and total liabilities';
  ...
  return <div className={s.profile}>
    <section className={s.overview} aria-labelledby="risk-profile-title">
      ...
      <aside className={s.balance} aria-labelledby="balance-structure-title">
        ...
        <div className={s.fundingComparison}><h3>{comparisonTitle}</h3><p>{bank ? 'Reported balances on a common dollar scale' : 'Separate reported balances · cash restrictions may apply'}</p>{comparisons.map(row => <div key={row.label} className={s.fundingRow}><div><span>{row.label}</span><strong>{formatRiskValue(row.item.value)}</strong></div><div className={s.fundingTrack}>{isNumber(row.item.value) && row.item.value >= 0 && <span style={{width:`${row.item.value/maxBalance*100}%`}}/>}</div></div>)}</div>
