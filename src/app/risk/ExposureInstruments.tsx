...
import { buildExposureInstrumentGroups, compactInstrumentLabel, instrumentComparisonScale } from '../../utils/exposureInstruments.js';
...
type Notes = { status: string; ticker: string; checkedAt: string; filing?: { form: string; filed: string; reportDate: string; accession: string; url: string }; rows: Row[]; coverage?: { rowsOmitted?: number }; limitations?: string[]; message?: string };
...
function ComparisonChart({ rows, concentration, fairValue = false, balanceLabel }: { rows: Row[]; concentration: boolean; fairValue?: boolean; balanceLabel?: string }) {
  ...
  const scale = instrumentComparisonScale(rows, concentration);
  const signed = scale.minimum < 0;
  ...
  return <>
    <div className={s.chartHeader}>
      ...
      <span className={s.scaleLabel}>{concentration ? `Share of ${balanceLabel?.toLowerCase() || 'reported credit balance'}` : fairValue ? 'Fair value / reported adjustment · USD' : 'Contract notional · USD'}</span>
    </div>
    <div className={s.chart} role="group" aria-label={concentration ? `${balanceLabel || 'Credit balance'} concentration comparison` : fairValue ? `${balanceLabel || 'Derivative fair value'} comparison` : 'Derivative notional comparison'}
      ...
      {pageRows.map((row, index) => {
        ...
        return <button key={row.id} type="button" ref={node => { controls.current[row.id] = node; }} className={`${s.chartRow} ${selected.id === row.id ? s.activeRow : ''}`}
          ...
          <span className={s.barTrack} aria-hidden="true">
            {signed && <span className={s.zeroLine} style={{ left: `${scale.zero}%` }}/>}
            <span className={s.currentBar} style={scale.bar(row.current.value)}/>
            {row.prior && <span className={s.priorBar} style={scale.bar(row.prior.value)}/>}
          </span>
      ...
      <div className={`${s.axis} ${signed ? s.signedAxis : ''}`} aria-hidden="true"><span>{concentration ? '0%' : formatValue(rows[0], scale.minimum)}</span><span style={signed ? { left: `${scale.zero}%` } : undefined}>{concentration ? '50%' : signed ? '$0' : formatValue(rows[0], scale.maximum / 2)}</span><span>{concentration ? '100%' : formatValue(rows[0], scale.maximum)}</span></div>
    </div>
    ...
    <div className={s.readout} aria-live="polite" aria-atomic="true">
      ...
      <dl className={s.readoutValues}>
        ...
        <div><dt>{concentration ? 'Share change' : fairValue ? 'Value change' : 'Notional change'}</dt><dd>{changeLabel(selected)}</dd><small>{selected.prior ? 'Matching concept and dimensions' : 'Unavailable for this measure'}</small></div>
      </dl>
    </div>
  </>;
}
...
function GroupView({ group }: { group: Group }) {
  ...
  return <>
    <div className={s.groupHeading}><div><h3>{group.label}</h3><p>{group.description}</p></div>{group.balances && group.balances.length > 1 && <label className={s.balanceSelect}>{group.kind === 'fair_value' ? 'Balance-sheet side' : 'Credit balance'}<select value={balance?.id} onChange={event => setBalanceId(event.target.value)}>{group.balances.map(item => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>}</div>
    <ComparisonChart key={balance?.id || group.id} rows={rows} concentration={group.kind === 'concentration'} fairValue={group.kind === 'fair_value'} balanceLabel={balance?.label}/>
    <p className={s.interpretation}>{group.kind === 'concentration' ? 'Each share uses the named credit balance. Counterparties and groups may overlap; shares are not added together.' : group.kind === 'fair_value' ? 'Positive bars show reported values; negative bars show reported reductions. Assets are receivable values and liabilities are payable values. Netting and collateral affect presentation; these amounts are not notionals, potential future exposure or a complete measure of counterparty loss. Rows may overlap and are not added.' : 'Notional measures contract scale, not potential loss or net exposure. Totals, components and hedge designations may overlap.'}</p>
  </>;
}
...
function Instruments({ ticker, basis = 'ttm', asOf = '' }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/risk/notes?${params}`, { signal: controller.signal }).then(async response => {
      ...
      if (!response.ok || !matchesRiskNoteResponse(body, ticker, basis, asOf)) throw new Error(body?.message || body?.error || 'The SEC filing notes could not be verified for this company and reporting selection. Please retry.');
  ...
  return <section ref={root} className={s.section} aria-label="Reported instruments and credit concentrations">
    <div className={s.heading}><div><span className={s.eyebrow}>SEC filing notes</span><h2>Instruments & counterparties</h2><p>Explore {ticker}’s derivative values, contract amounts and disclosed credit concentrations.</p></div>{data?.filing && <a className={s.filingLink} href={data.filing.url} target="_blank" rel="noreferrer">{data.filing.form} · {data.filing.reportDate}<ArrowUpRight size={15}/></a>}</div>
    ...
    {data && <>
      ...
      </> : <p className={s.state}>This filing has no supported dollar notional, derivative fair-value or credit-concentration facts in the available extraction. This is a coverage gap, not evidence of zero exposure.</p>}
      {!!data.coverage?.rowsOmitted && <p className={s.interpretation}>{data.coverage.rowsOmitted} additional tagged measures exceed this view’s display limit. Open the SEC filing for complete coverage.</p>}
      <details className={s.details}><summary>Source coverage & interpretation</summary><p>Standard USD derivative notionals, fair values and netting adjustments, plus credit-concentration percentages from one filing. Missing amounts do not establish zero exposure.</p>{data.limitations?.map(note => <p key={note}>{note}</p>)}{data.filing && <p>Filed {data.filing.filed} · SEC accession {data.filing.accession} · Checked {data.checkedAt?.slice(0, 10)}</p>}</details>
    </>}
  </section>;
}
