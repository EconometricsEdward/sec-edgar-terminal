...
function MarketContext({ market, ticker, asOf = '' }: Props) {
  ...
  const inspectedDate = inspection?.path === path ? inspection.date : selected?.reportDate;
  ...
  const fourWeekly = useMemo(() => cftcPositionChange(inspectedHistory, 4), [inspectedHistory]);
  ...
  const marketUrl = `/market?${new URLSearchParams({ tab: 'positioning', family: market.family, contract: market.contract, group, history: '1y', display: 'net-oi', ...(inspectedDate ? { date: inspectedDate } : {}) })}`;
  ...
  return <section className={s.root} aria-label={`${market.label} CFTC market observations`}>
    <header className={s.header}>
      <div><span className={s.eyebrow}><ChartNoAxesCombined size={15} /> Market positioning</span><h3>{market.label}</h3><p>{family?.label} · Futures only · Contract {market.contract}</p></div>
      <label>Trader category<select value={group} onChange={event => { setGroup(event.target.value); setInspection(null); }}>{family?.groups.map(item => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>
    </header>
    <p className={s.scope}>{market.fit === 'user-selected' ? 'Independent market reference.' : 'Context for a disclosed business exposure.'} Aggregate CFTC positions; {ticker}’s own holdings and hedge coverage are outside this dataset.</p>
    {asOf && <p className={s.notice}>SEC evidence is limited to filings through {asOf}. CFTC observations use the latest available report, independently of that cutoff.</p>}
    {loading && <p className={s.loading} role="status"><Loader2 className={s.spin} size={16} />Loading the selected CFTC market…</p>}
    {error && <div className={s.notice} role="alert"><p>{error} The SEC exposure evidence remains available.</p><button type="button" onClick={() => { clearPreparedCftc(path); setInspection(null); setRetry(count => count + 1); }}><RefreshCw size={14} />Retry market data</button></div>}
    {history && !loading && !error && <>
      <div className={s.dates}><span>Positions as of <strong>{inspectedDate}</strong></span><span>Retrieved <strong>{history.retrieved_at?.slice(0, 16).replace('T', ' ')} UTC</strong></span></div>
      ...
      <dl className={s.metrics}>
        <div><dt>Net position / open interest</dt><dd>{signed(inspectedPoint?.netPctOi, 1, '%')}</dd><small>{signed(inspectedPoint?.net)} net contracts</small></div>
      ...
      <CftcPositioningVisuals layout="compact" history={history} onGroupChange={next => { setGroup(next); setInspection(null); }} onReportInspect={date => setInspection({ path, date })} />
      ...
      <footer className={s.footer}><details className={s.methodology}><summary>Market scope & methodology</summary><p>{market.basisLimit}</p><p>Net = longs − shorts. Historical rank compares net / open interest with 52 earlier reports, using half weight for ties. Changes describe contracts outstanding; they do not measure company exposure, cash flows, or future prices.</p></details><div className={s.links}><a href={marketUrl}>Open dated contract history <ArrowUpRight size={14} /></a><a href={history.source?.url} target="_blank" rel="noreferrer">Official CFTC source <ArrowUpRight size={14} /></a></div></footer>
    </>}
  </section>;
}
