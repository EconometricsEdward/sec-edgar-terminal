...
function MarketChart({ ticker, market, asOf }: { ticker: string; market: Benchmark; asOf: string }) {
  ...
  const focusDate = inspectedDate || baselineDate;
  const focus = chart?.positions.find(point => point.reportDate === focusDate);
  const changes = useMemo(() => [1, 4, 13].map(weeks => ({ weeks, ...cftcPositionChange(history ? { ...history, selected: { ...history.selected, reportDate: focusDate } } : null, weeks) })), [history, focusDate]);
  const extent = signedPlotExtent(changes.map(item => item.available ? item.netPctChange : null));
  const current = history?.selected;
  const marketUrl = `/market?${new URLSearchParams({ tab: 'positioning', family: market.family, contract: market.contract, group, history: '1y', display: 'net-oi', ...(focusDate ? { date: focusDate } : {}) })}`;
  ...
  return <div className={s.market}>
    <div className={s.chartHeading}><div><span className={s.kicker}>{market.fit === 'user-selected' ? 'Independent CFTC reference' : 'Filing-linked CFTC context'}</span><h3>{market.label}</h3><p>{family.label} · Futures only</p></div><label>Trader category<select value={group} onChange={event => { setGroup(event.target.value); setSelectedDate(''); setInspectedDate(null); }}>{family.groups.map(item => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label></div>
    ...
    {error && <div className={s.notice} role="status"><p>{error}</p><button type="button" onClick={() => { clearPreparedCftc(path); setSelectedDate(''); setInspectedDate(null); setRetry(value => value + 1); }}><RefreshCw size={13} />Retry CFTC history</button></div>}
    {history && !pending && !error && <>
      ...
      <div className={s.chartTools}><div className={s.switcher} aria-label="Profile market chart measure"><button type="button" aria-pressed={measure === 'net'} onClick={() => { setMeasure('net'); setInspectedDate(null); }}>Net positioning</button><button type="button" aria-pressed={measure === 'interest'} onClick={() => { setMeasure('interest'); setInspectedDate(null); }}>Open interest</button></div><span>Inspecting {focusDate || current.reportDate}</span></div>
      ...
      {chart ? <div className={s.chartViewport} tabIndex={0} role="region" aria-label="CFTC history chart; scroll horizontally on small screens"><svg className={s.chart} viewBox="0 0 820 260" role="group" aria-label={`${market.label}, ${current.selectedGroup?.label}: ${measure === 'net' ? 'net positions as a share of market open interest' : 'market open interest'}, ${chart.start} to ${chart.end}. Exact dated values follow in a collapsible table.`}>{chart.ticks.map((tick, index) => <g key={index}><line x1={68} x2={792} y1={tick.y} y2={tick.y} className={s.grid} /><text x={55} y={tick.y + 4} textAnchor="end">{measure === 'net' ? `${number(tick.value, 1)}%` : compact(tick.value)}</text></g>)}<line x1={68} x2={792} y1={chart.zeroY} y2={chart.zeroY} className={s.zero} />{chart.series.map(series => <g key={series.field}>{series.paths.map((path, index) => <path key={index} d={path} className={s.line} />)}{series.dots.map(dot => <circle key={dot.date} cx={dot.x} cy={dot.y} r={dot.date === focus?.reportDate ? 4 : 1.5} className={s.dot}><title>{dot.date}: {measure === 'net' ? signed(dot.value, 2, '%') : `${number(dot.value)} contracts`}</title></circle>)}</g>)}{focus && <line x1={focus.x} x2={focus.x} y1={30} y2={223} className={s.cursor} />}<text x={68} y={252}>{chart.start}</text><text x={792} y={252} textAnchor="end">{chart.end}</text><ChartPeriodOverlay points={chart.positions.map(point => ({ id: point.reportDate, label: point.reportDate, x: point.x }))} activeId={focusDate} onInspect={setInspectedDate} left={68} right={792} top={28} bottom={224} label={`${market.label} report inspection`} describePoint={date => { const point = chart.positions.find(item => item.reportDate === date); return `${date}: net ${signed(point?.netPctOi, 2, '%')}, ${signed(point?.net, 0)} net contracts; ${number(point?.openInterest)} open interest`; }} /></svg></div> : <p className={s.status}>At least two compatible dated observations are needed for a chart.</p>}
      {chart && <div className={s.inspectionTools}><span>Hover, tap, or use arrow keys to inspect a report.</span><label className={s.inspect}>Inspect report<select value={baselineDate} onChange={event => { setSelectedDate(event.target.value); setInspectedDate(null); }}>{chart.positions.slice().reverse().map(point => <option key={point.reportDate} value={point.reportDate}>{point.reportDate}</option>)}</select></label></div>}
      <div className={s.shifts}><div className={s.shiftHeading}><strong>Positioning shifts</strong><span>Through {focusDate} · percentage points in net / OI</span></div>{changes.map(item => <div className={s.shift} key={item.weeks}><span>{item.weeks} week{item.weeks > 1 ? 's' : ''}</span><div className={s.shiftTrack} aria-hidden="true">{item.available && finite(item.netPctChange) && <i data-negative={item.netPctChange < 0} style={{ left: `${item.netPctChange < 0 ? 50 + item.netPctChange / extent * 50 : 50}%`, width: `${Math.abs(item.netPctChange) / extent * 50}%` }} />}</div><strong>{signed(item.available ? item.netPctChange : null, 2, ' pp')}</strong></div>)}<p className={s.caption}>Changes use exact 1-, 4- and 13-week comparisons on a common ±{extent} pp scale. Gaps remain unavailable. Positioning is market context, not {ticker}’s exposure or a price forecast.</p></div>
