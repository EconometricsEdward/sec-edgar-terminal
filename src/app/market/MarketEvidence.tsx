  ...
  return <dialog ref={dialog} className={s.dialog} onClose={onClose} aria-labelledby="market-evidence-title" onClick={(e) => { if (e.target === e.currentTarget) dialog.current?.close(); }}>
    <div className={s.dialogBody}>
      ...
      {data && <>
        ...
        <p className={s.note}>{data.revenueBasis}. Retrieved {data.observedAt.slice(0, 16).replace('T', ' ')} UTC. Raw inputs retain their actual source dates and values. Financial-company cash flows and capital structures require sector-specific interpretation.</p>
      </>}
    </div>
  </dialog>;
}
