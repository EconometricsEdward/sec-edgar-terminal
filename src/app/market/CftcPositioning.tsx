...
const DEFAULTS: Record<string, { contract: string; group: string }> = { tff: { contract: '13874A', group: 'leveraged-funds' }, disaggregated: { contract: '067651', group: 'managed-money' } };
const fmt = (value: unknown, digits = 1) => typeof value === 'number' && Number.isFinite(value) ? value.toLocaleString('en-US', { maximumFractionDigits: digits }) : '—';
const signed = (value: unknown, suffix = '') => typeof value === 'number' && Number.isFinite(value) ? `${value > 0 ? '+' : ''}${fmt(value, 1)}${suffix}` : '—';
...
function SeriesChart({ points, field, title, formatter }: { points: any[]; field: string; title: string; formatter: (value: unknown) => string }) {
  const numeric = points.filter(point => typeof point[field] === 'number' && Number.isFinite(point[field]));
  if (numeric.length < 2) return <p className={s.empty}>This chart needs at least two compatible observations.</p>;
  ...
  points.forEach((point, index) => {
    if (typeof point[field] !== 'number') { if (path) segments.push(path); path = ''; return; }
    path += `${path ? ' L' : 'M'} ${x(index)} ${y(point[field])}`;
  });
  if (path) segments.push(path);
  return <div className={s.chart}><div className={s.chartHeading}><h3>{title}</h3><span>{formatter(numeric.at(-1)?.[field])} latest</span></div><svg viewBox="0 0 580 205" role="img" aria-label={`${title}, ${numeric.length} reported observations`}><line x1="24" x2="556" y1="172" y2="172" /><line x1="24" x2="556" y1="40" y2="40" />{segments.map((segment, index) => <path key={index} d={segment} />)}<text x="24" y="194">{points[0]?.reportDate}</text><text x="465" y="194">{points.at(-1)?.reportDate}</text><text x="28" y="34">{formatter(max)}</text><text x="28" y="168">{formatter(min)}</text></svg></div>;
}

export default function CftcPositioning({ view, onView, onNotice }: Props) {
  const [markets, setMarkets] = useState<any>(null), [historyState, setHistoryState] = useState<{ key: string; data: any } | null>(null);
  ...
  useEffect(() => {
    ...
    const path = `/api/v1/cftc/markets?family=${encodeURIComponent(family)}`;
    fetchPreparedCftc(path, { signal: controller.signal }).then(result => { if (active) setMarkets(result); }).catch(reason => { if (active) setError(reason.message); }).finally(() => { if (active) setLoading(false); });
    return () => { active = false; controller.abort(); };
  }, [family, retry]);

  const catalog = useMemo(() => (markets?.catalog || []).filter((item: any) => item.code === view.cftcContract || !query.trim() || `${item.marketName} ${item.exchange} ${item.code} ${item.commodity}`.toLowerCase().includes(query.trim().toLowerCase())), [markets, query, view.cftcContract]);
  const knownContract = markets?.catalog?.some((item: any) => item.code === view.cftcContract);
  const contract = knownContract ? view.cftcContract : markets?.catalog?.find((item: any) => item.code === DEFAULTS[family].contract)?.code || markets?.catalog?.[0]?.code || '';
  ...
  useEffect(() => {
    if (!markets || !contract || !validGroup) return;
    const patch: Partial<MarketView> = {};
    if (view.cftcContract !== contract) patch.cftcContract = contract;
    if (view.cftcGroup !== validGroup) patch.cftcGroup = validGroup;
    if (Object.keys(patch).length) onView(patch);
  }, [markets, contract, validGroup, view.cftcContract, view.cftcGroup, onView]);

  useEffect(() => {
    if (!markets || !contract || !validGroup) return;
    let active = true; const controller = new AbortController(); setHistoryLoading(true); setHistoryError('');
    const params = new URLSearchParams({ family, contract, group: validGroup, date: view.cftcDate, window: view.cftcHistory });
    fetchPreparedCftc(`/api/v1/cftc/history?${params}`, { signal: controller.signal }).then(result => { if (active) setHistoryState({ key: historyKey, data: result }); }).catch(reason => { if (active) { setHistoryState(null); setHistoryError(reason.message); } }).finally(() => { if (active) setHistoryLoading(false); });
    return () => { active = false; controller.abort(); };
  }, [markets, family, contract, validGroup, view.cftcDate, view.cftcHistory, historyKey]);

  const selectedMarket = markets?.catalog?.find((item: any) => item.code === contract);
  const groups = markets?.groups || [];
  const latestRows = markets?.latest || [];
  const familyConfig: any = CFTC_FAMILIES[family];

  function changeFamily(nextFamily: string) {
    const defaults = DEFAULTS[nextFamily];
    setMarkets(null); setHistoryState(null); setQuery('');
    onView({ cftcFamily: nextFamily as MarketView['cftcFamily'], cftcContract: defaults.contract, cftcGroup: defaults.group, cftcDate: 'latest' }, true);
  }
  ...
  return <section className={s.workspace} aria-label="CFTC Commitments of Traders positioning">
    ...
    {error && <div className={s.error} role="alert"><AlertTriangle /><div><b>CFTC positioning is temporarily unavailable</b><span>{error}</span></div><button onClick={() => { clearPreparedCftc(); setRetry(value => value + 1); }}><RefreshCw size={14} />Retry</button></div>}
    {markets && <>
      {(markets.status === 'stale' || markets.status === 'partial') && <p className={s.warning} role="status"><AlertTriangle size={15} /><span><b>{markets.status === 'stale' ? 'Showing the last completed CFTC snapshot.' : 'This CFTC snapshot has disclosed coverage or validation limits.'}</b> {markets.refresh_warning || `${markets.coverage.launch_available} of ${markets.coverage.launch_expected} launch markets are represented; ${markets.coverage.required_values_unavailable?.length || 0} launch rows have unavailable required values.`}</span></p>}
      <div className={s.snapshotBar}><span><b>{familyConfig.shortLabel}</b> · Futures only</span><span>Report date <b>{markets.report_date}</b></span><span><b>{markets.catalog.length}</b> current contracts</span><span>Retrieved <b>{String(markets.retrieved_at).slice(0, 16).replace('T', ' ')} UTC</b></span></div>
      <div className={s.controls}>
        <label className={s.search}><span>Find a contract</span><div><Search size={15} /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Name, exchange, commodity, or code" /></div></label>
        <label><span>Verified contract</span><select value={contract} onChange={event => selectContract(event.target.value)}>{Object.entries(CFTC_CATEGORY_LABELS).map(([category, label]) => { const options = catalog.filter((item: any) => item.category === category); return options.length ? <optgroup key={category} label={label}>{options.map((item: any) => <option key={item.code} value={item.code}>{item.launch ? '★ ' : ''}{item.marketName} · {item.code}</option>)}</optgroup> : null; })}</select></label>
        ...
        <label><span>History</span><select value={view.cftcHistory} onChange={event => onView({ cftcHistory: event.target.value as MarketView['cftcHistory'] }, true)}><option value="1y">52 prior reports</option><option value="3y">156 prior reports</option><option value="5y">260 prior reports</option></select></label>
      ...
      <section className={s.panel}><div className={s.sectionHeading}><div><span className={s.eyebrow}>{familyConfig.shortLabel} heatmap</span><h2>Positioning across the launch selection</h2><p>{view.cftcDisplay === 'net-oi' ? 'Net contracts as a percentage of each market’s own open interest.' : 'Within-contract 260-prior-report percentile. Unavailable means the full comparable history is not present; this is not a cross-market percentile.'}</p></div><span className={s.legend}>{view.cftcDisplay === 'percentile' ? <><i data-tone="low" />Lower historical rank <i />Unavailable <i data-tone="high" />Higher historical rank</> : <><i />Net short <i />Unavailable <i />Net long</>}</span></div>{latestRows.length ? <div className={s.tableWrap} role="region" aria-label={`${familyConfig.shortLabel} positioning heatmap`} tabIndex={0}><table className={s.heatmap}><caption>{familyConfig.label} positioning heatmap with numeric table values</caption><thead><tr><th scope="col">Contract</th>{groups.map((group: any) => <th scope="col" key={group.id}>{group.label}</th>)}</tr></thead><tbody>{latestRows.map((row: any) => <tr key={row.code}><th scope="row"><button onClick={() => selectContract(row.code)}>{row.launchLabel || row.contractName}<small>{row.exchange} · {row.code}</small></button></th>{groups.map((groupDef: any) => { const value = view.cftcDisplay === 'percentile' ? row.groups[groupDef.id]?.percentile?.value : row.groups[groupDef.id]?.netPctOi; const sign = value == null ? 'missing' : view.cftcDisplay === 'percentile' ? value < 33 ? 'low' : value > 66 ? 'high' : 'middle' : value >= 0 ? 'positive' : 'negative'; return <td key={groupDef.id}><button data-sign={sign} style={heatStyle(value, view.cftcDisplay)} onClick={() => { onView({ cftcContract: row.code, cftcGroup: groupDef.id, cftcDate: 'latest' }, true); }} aria-label={`${row.launchLabel || row.contractName}, ${groupDef.label}: ${value == null ? 'unavailable' : `${fmt(value)} percent`}`}>{value == null ? '—' : `${fmt(value)}%`}<small>{view.cftcDisplay === 'percentile' ? `${row.groups[groupDef.id]?.percentile?.observations || 0} prior` : signed(row.groups[groupDef.id]?.oneWeekNetPctChange, ' pp weekly')}</small></button></td>; })}</tr>)}</tbody></table></div> : <p className={s.empty}>No launch-selection rows are available in this partial snapshot. Use the verified contract catalog above for available markets.</p>}</section>

      <section className={s.panel}><div className={s.sectionHeading}><div><span className={s.eyebrow}>Latest report table</span><h2>{groups.find((item: any) => item.id === validGroup)?.label} across the {familyConfig.shortLabel} launch selection</h2><p>Compatible changes require a report exactly seven days earlier. Missing is never displayed as zero.</p></div></div>{latestRows.length ? <div className={s.tableWrap} role="region" aria-label={`${familyConfig.shortLabel} latest positioning table`} tabIndex={0}><table><thead><tr><th scope="col">Contract</th><th scope="col">Open interest</th><th scope="col">Long</th><th scope="col">Short</th><th scope="col">Net</th><th scope="col">Net / OI</th><th scope="col">Weekly Δ net (contracts)</th><th scope="col">260-prior-report rank</th></tr></thead><tbody>{latestRows.map((row: any) => { const group = row.groups[validGroup]; return <tr key={row.code}><th scope="row"><button className={s.textButton} onClick={() => selectContract(row.code)}>{row.launchLabel || row.contractName}</button><small>{row.code} · {row.exchange}</small></th><td>{fmt(row.openInterest, 0)}</td><td>{fmt(group?.long, 0)}</td><td>{fmt(group?.short, 0)}</td><td>{signed(group?.net)}</td><td>{signed(group?.netPctOi, '%')}</td><td>{signed(group?.oneWeekChange)}</td><td>{group?.percentile?.value == null ? <span title={`Needs ${group?.percentile?.required || 260} prior valid reports`}>Insufficient</span> : `${fmt(group.percentile.value)}%`}<small>{group?.percentile?.observations || 0} prior reports</small></td></tr>; })}</tbody></table></div> : <p className={s.empty}>No launch-selection rows are available for this family.</p>}</section>

      <section className={s.detail}><div className={s.detailHead}><div><span className={s.eyebrow}>Contract detail</span><h2>{selectedMarket?.marketName || contract}</h2><p>{selectedMarket?.exchange} · CFTC code {contract} · {selectedMarket?.units || 'Units unavailable'}</p></div><div className={s.actions}><button disabled={!history} onClick={exportCsv}><Download size={14} />CSV</button><button disabled={!history} onClick={exportJson}><Download size={14} />JSON</button></div></div>
        {historyLoading && <p className={s.inlineState} role="status"><Loader2 className={s.spin} size={17} />Loading bounded contract history…</p>}
        {historyError && <p className={s.warning} role="alert"><AlertTriangle size={15} />{historyError}</p>}
        {history && <>
          ...
          <div className={s.charts}><SeriesChart points={history.history} field="netPctOi" title="Net position / open interest" formatter={value => signed(value, '%')} /><SeriesChart points={history.history} field="openInterest" title="Open interest" formatter={value => fmt(value, 0)} /></div>
          <details className={s.dataTable}><summary>Accessible history table · {history.history.length} observations</summary><div className={s.tableWrap} role="region" aria-label="CFTC contract history" tabIndex={0}><table><thead><tr><th scope="col">Report date</th><th scope="col">Open interest</th><th scope="col">Long</th><th scope="col">Short</th><th scope="col">Spreading</th><th scope="col">Net</th><th scope="col">Net / OI</th></tr></thead><tbody>{[...history.history].reverse().map((point: any) => <tr key={point.reportDate}><th scope="row">{point.reportDate}</th><td>{fmt(point.openInterest, 0)}</td><td>{fmt(point.long, 0)}</td><td>{fmt(point.short, 0)}</td><td>{point.spreading == null ? 'Not available / applicable' : fmt(point.spreading, 0)}</td><td>{signed(point.net)}</td><td>{signed(point.netPctOi, '%')}</td></tr>)}</tbody></table></div></details>
...
export function CftcPositioningPreview({ onOpen }: { onOpen: () => void }) {
  const [state, setState] = useState<any>({ pending: 2, tff: null, disaggregated: null });
  useEffect(() => {
    let active = true;
    const controllers = [new AbortController(), new AbortController()];
    const load = (family: 'tff' | 'disaggregated', controller: AbortController) => {
      fetchPreparedCftc(`/api/v1/cftc/markets?family=${family}`, { signal: controller.signal })
        .then(value => { if (active) setState((current: any) => ({ ...current, [family]: value })); })
        .catch(() => {})
        .finally(() => { if (active) setState((current: any) => ({ ...current, pending: current.pending - 1 })); });
    };
    load('tff', controllers[0]); load('disaggregated', controllers[1]);
    return () => { active = false; controllers.forEach(controller => controller.abort()); };
  }, []);
  const hasData = state.tff || state.disaggregated;
  return <section className={s.preview}><div><span className={s.eyebrow}>CFTC positioning</span><h2>Futures positioning, alongside—not inside—company research.</h2><p>Official futures-only COT reports provide separate market context across financial and physical contracts.</p></div>{hasData ? <div className={s.previewStats}>{state.tff && <span><b>{state.tff.report_date}</b>TFF · {state.tff.catalog.length} contracts</span>}{state.disaggregated && <span><b>{state.disaggregated.report_date}</b>Disaggregated · {state.disaggregated.catalog.length} contracts</span>}{state.pending > 0 && <span role="status"><Loader2 className={s.spin} size={14} />Loading the other report family…</span>}</div> : state.pending > 0 ? <p role="status"><Loader2 className={s.spin} size={16} />Loading the CFTC preview…</p> : <p>CFTC preview unavailable. SEC research remains unaffected.</p>}<button onClick={onOpen}>Open CFTC Positioning</button></section>;
}
