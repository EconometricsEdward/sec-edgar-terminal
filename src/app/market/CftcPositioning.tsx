...
import { selectCftcHistoryCoverage } from '../../utils/cftcHistoryCoverage.js';
...
const STATUS_PATH = '/api/v1/cftc/status';
...
export default function CftcPositioning({ view, onView, onNotice }: Props) {
  ...
  const [coverageState, setCoverageState] = useState<any>(null);
  ...
  useEffect(() => {
    ...
    setMarketsState(null); setCoverageState(null); setLoading(true); setError('');
    ...
    // Reuse the bounded shared client cache. Coverage does not poll or block research.
    fetchPreparedCftc(STATUS_PATH, { signal: controller.signal, timeoutMs: 5000 }).then(result => { if (active) setCoverageState(result); }).catch(() => {});
    return () => { active = false; controller.abort(); };
  }, [marketsPath, marketsRetry]);

  const markets = marketsState?.report_family === family ? marketsState : null;
  const historyCoverage = selectCftcHistoryCoverage(coverageState?.history_coverage, family, markets?.report_date, markets?.catalog?.length);
  ...
  function retryMarkets() { clearPreparedCftc(marketsPath); clearPreparedCftc(STATUS_PATH); setMarketsRetry(value => value + 1); }
  ...
  return <section className={s.workspace} aria-label="CFTC Commitments of Traders positioning">
    ...
    {loading && <div className={s.state} role="status"><Loader2 className={s.spin} /><div><b>Loading the prepared CFTC snapshot</b><span>Catalog, latest positions, and contract history are independent of SEC data.</span></div></div>}
    ...
    {markets && <>
      ...
      {markets.status === 'partial' && <p className={s.warning} role="status"><AlertTriangle size={15} /><span><b>This CFTC snapshot has disclosed coverage or validation limits.</b> {markets.refresh_warning || `${markets.coverage.launch_available} of ${markets.coverage.launch_expected} heatmap markets are represented; ${markets.coverage.required_values_unavailable?.length || 0} heatmap rows have unavailable required values.`}</span></p>}
      ...
      <div className={s.coverageBar} aria-label={`${familyConfig.shortLabel} snapshot coverage`}><span><b>{markets.catalog.length}</b> verified current contracts</span><span><b>{markets.coverage.launch_available} / {markets.coverage.launch_expected}</b> heatmap markets available</span><span><b>{markets.coverage.required_values_unavailable?.length || 0}</b> heatmap rows missing required values</span><span><b>{markets.coverage.quarantined_rows || 0}</b> quarantined source rows</span></div>
      {historyCoverage && <section className={s.historyCoverage} aria-label={`${familyConfig.shortLabel} historical coverage`}>
        <div><span className={s.eyebrow}><Database size={14} />Historical coverage</span><h3>{fmt(historyCoverage.contracts_prepared, 0)} of {fmt(historyCoverage.contracts_total, 0)} contracts prepared</h3><p>{familyConfig.shortLabel} · Futures only · Through {historyCoverage.report_date}. History length varies by contract; a prepared chart may have fewer reports than your selected percentile requires.</p></div>
        <div className={s.historyCoverageCounts}><span><b>{fmt(historyCoverage.contracts_limited, 0)}</b> prepared with limited history</span><span><b>{fmt(historyCoverage.contracts_pending, 0)}</b> still preparing</span><span><b>{fmt(historyCoverage.contracts_unavailable, 0)}</b> currently unavailable</span></div>
        <small>Coverage checked {retrieved(coverageState?.checked_at)}. Limited history includes shorter records and disclosed source or validation limits.</small>
      </section>}
      ...
      <section className={s.detail}><div className={s.detailHead}><div><span className={s.eyebrow}>Contract detail</span><h2>{selectedMarket?.marketName || `Unsupported contract ${view.cftcContract}`}</h2><p>{selectedMarket ? `${selectedMarket.exchange} · CFTC code ${contract} · ${selectedMarket.units || 'Units unavailable'}` : 'Select a verified contract above. No substitute contract has been loaded.'}</p></div><div className={s.actions}><button disabled={!history} onClick={exportCsv}><Download size={14} />CSV</button><button disabled={!history} onClick={exportJson}><Download size={14} />JSON</button></div></div>
        ...
        {historyError && <div className={s.warning} role="alert"><AlertTriangle size={15} /><span>{historyError}</span><button onClick={retryHistory}><RefreshCw size={14} />Retry this history</button>{view.cftcDate !== 'latest' && <button onClick={() => onView({ cftcDate: 'latest' }, true)}>Return to latest report</button>}</div>}
        {history && <>
