...
import { MARKET_ATLAS_FRESH_MS, MARKET_SAVED_KEY, parseMarketView, marketViewQuery, parseMarketSaved, selectMarketCompanies, marketCsv, marketBrief } from '../../utils/marketResearch.js';
...
export default function MarketOverviewClient({ initialData = null, initialQuery = '' }: { initialData?: MarketData | null; initialQuery?: string }) {
  const [data, setData] = useState<MarketData | null>(initialData);
  const [loading, setLoading] = useState(!initialData);
  const [slow, setSlow] = useState(false);
  const [error, setError] = useState('');
  const [retry, setRetry] = useState(0);
  const [view, setView] = useState<MarketView>(() => parseMarketView(initialQuery, COHORT_IDS) as MarketView);
  ...
  const isCftcTab = view.tab === 'positioning';
  ...
  useEffect(() => {
    ...
    const restoreSaved = () => {
      try {
        const restored = parseMarketSaved(localStorage.getItem(MARKET_SAVED_KEY)) as Saved;
  ...
  function updateView(patch: Partial<MarketView>, push = false) {
    ...
    window.history[push ? 'pushState' : 'replaceState'](null, '', `/market${query ? `?${query}` : ''}`);
  }
  ...
  return <div className={s.page}>
    ...
    {data && <>
      {view.tab === 'overview' && <><Briefing companies={cohortCompanies} data={data} basis={view.basis} onScreen={screen} /><CftcPositioningPreview onOpen={() => updateView({ tab: 'positioning' }, true)} /><div className={s.sectionLink}><div><h2>From the market to a company</h2><p>Choose a sector below, or use the screener to build your own research list.</p></div><button className={s.primary} onClick={() => updateView({ tab: 'companies', screen: 'all', query: '' }, true)}>Explore companies <ArrowUpRight size={16} /></button></div><SectorMap data={data} basis={view.basis} statistic={view.statistic} selectedCohort={view.cohort} onStatistic={(statistic) => updateView({ statistic })} onCohort={(cohort, metric) => updateView({ cohort, tab: 'companies', query: '', screen: 'all', metric: metric || view.metric, sort: metric || view.sort }, true)} /><ObservationHistory data={data} /></>}
