...
import { updateMarketView } from '../../utils/marketOverview.js';
import { isMarketBriefing } from '../../utils/marketResearchValidation.js';
...
import type { MarketBriefingData, MarketView } from './marketTypes';
...
export default function MarketOverviewClient({ initialData = null, initialQuery = '', cftcEnabled = true }: { initialData?: MarketBriefingData | null; initialQuery?: string; cftcEnabled?: boolean }) {
  const [data, setData] = useState<MarketBriefingData | null>(initialData);
  ...
  useEffect(() => {
    const restore = () => {
      ...
      setView(restored); setShareFallback(''); setLocationReady(true);
  ...
  useEffect(() => {
    // Restore the URL first. A direct CFTC visit does not request SEC coverage.
    if (!locationReady || isCftcTab) return;
    ...
    async function load() {
      try {
        const response = await fetch('/api/market-briefing', { signal: controller.signal });
        const result = await response.json();
        if (!response.ok || !isMarketBriefing(result)) throw new Error(result.error || 'Sector fundamentals are temporarily unavailable.');
  ...
  const summary = data?.summaries[view.basis] || null;
  const sectorProps = {
    summary: summary!, basis: view.basis, statistic: view.statistic, selectedSector: view.cohort, metric: view.metric,
  ...
  return <div className={s.page}>
    ...
    {locationReady && !isCftcTab && loading && !data && <div className={s.loading} role="status"><Loader2 className={s.spin} size={25} /><div><h2>{slow ? 'Retrieving the prepared sector snapshot' : 'Loading the market universe'}</h2><p>Loading sector coverage and reported fundamentals. {cftcEnabled && 'CFTC reports load independently.'}</p></div></div>}
    {!isCftcTab && error && <div className={s.error} role="alert"><h2>Sector snapshot unavailable</h2><p>{error}</p><button className={s.button} onClick={() => { setError(''); setRetry((n) => n + 1); }}><RefreshCw size={14} />Retry sector data</button></div>}
    {!isCftcTab && data?.cache?.status === 'stale' && <div className={s.warning} role="status"><RefreshCw size={16} /><p><b>Showing the last completed SEC snapshot.</b> {data.cache.warning} Calculated {data.generatedAt.slice(0, 16).replace('T', ' ')} UTC.</p></div>}
    {locationReady && view.tab === 'overview' && <>
      {summary && <MarketMacroBriefing summary={summary} basis={view.basis} cftcEnabled={cftcEnabled} onSector={(cohort) => updateView({ tab: 'sectors', cohort, metric: 'revenueGrowth', statistic: 'median' }, true)} />}
      {cftcEnabled && <MarketMacroPositioning onView={updateView} cftcEnabled={cftcEnabled} />}
      {summary && <MarketSectorPerformance {...sectorProps} compact />}
    </>}
    {isCftcTab && <CftcPositioning view={view} onView={updateView} onNotice={setNotice} />}
    {view.tab === 'sectors' && summary && <MarketSectorPerformance {...sectorProps} />}
    {!isCftcTab && <details className={`${s.panel} ${s.details}`}><summary>How to read this market view · methods & sources</summary><div className={s.methodology}>
      ...
      <p><b>Universe.</b> The prepared SEC issuer universe supplies the coverage; only loaded companies contribute to the figures. Each issuer has one primary sector. Industry counts use reported SEC SIC codes. Coverage lists and source links are available above.</p>
