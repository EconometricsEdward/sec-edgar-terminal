...
export default function MarketOverviewClient({ initialData = null }: { initialData?: MarketData | null }) {
  ...
  return <div className={s.page}>
    ...
    {data && <>
      ...
      <div className={s.toolbar}><label>{view.tab === 'factors' ? 'Peer normalization universe' : 'Research universe'}<select value={view.cohort} onChange={(e) => updateView({ cohort: e.target.value })}><option value="all">{view.tab === 'factors' ? 'Automatic company cohort' : 'All covered companies'}</option>{data.cohorts.map((c) => <option key={c.id} value={c.id}>{c.label}</option>)}</select></label><div className={s.actions}><button className={s.button} onClick={shareView}><Share2 size={14} />Share view</button><button className={s.button} onClick={() => { setViewName(view.tab === 'factors' ? `${view.factorTicker} Factor Lab · ${view.factorWindow.toUpperCase()} · ${view.basis === 'ttm' ? 'TTM' : 'Annual'}` : `${cohort?.label || 'Market'} · ${view.basis === 'ttm' ? 'TTM' : 'Annual'}`); setSaveOpen(!saveOpen); }}><Bookmark size={14} />Save view</button>{view.tab !== 'factors' && <><button className={s.button} onClick={() => { downloadText(`market-${view.basis}-screen.csv`, marketCsv(rows, view.basis, data.generatedAt), 'text/csv'); setNotice(`CSV exported: ${rows.length} companies in the current screen.`); }}><Download size={14} />Export CSV</button><button className={s.button} onClick={() => { downloadText('market-research-brief.md', marketBrief(rows, view, data, window.location.href), 'text/markdown'); setNotice('Research brief exported.'); }}><Download size={14} />Research brief</button></>}</div></div>
      ...
      {view.tab === 'factors' && <MarketFactorLab companies={companies} basis={view.basis} cohortId={view.cohort} factorTicker={view.factorTicker} factorWindow={view.factorWindow} factorSector={view.factorSector} onView={updateView} onNotice={setNotice} />}
