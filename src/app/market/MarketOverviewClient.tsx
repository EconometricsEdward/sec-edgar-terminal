...
import { MARKET_ATLAS_FRESH_MS, MARKET_SAVED_KEY, DEFAULT_MARKET_VIEW, parseMarketView, marketViewQuery, parseMarketSaved, selectMarketCompanies, marketCsv, marketBrief } from '../../utils/marketResearch.js';
...
export default function MarketOverviewClient({ initialData = null }: { initialData?: MarketData | null }) {
  ...
  useEffect(() => {
    ...
    if (isFactorTab) { setLoading(false); return; }
  ...
  function updateView(patch: Partial<MarketView>) {
    const next = updateMarketView(view, patch) as MarketView;
