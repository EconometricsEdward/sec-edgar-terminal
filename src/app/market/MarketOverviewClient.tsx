...
export default function MarketOverviewClient() {
  ...
  return <div className={s.page}>
    ...
    {data && <>
      ...
      <p className={s.basisNote}>{view.basis === 'ttm' ? 'Quarter-end balances · Trailing-twelve-month flows · Year-over-year revenue growth' : 'Annual balances and flows · Year-over-year revenue growth'} <span>Screen exports: {rows.length} companies{view.query ? ` · “${view.query}”` : ''}{view.screen !== 'all' ? ` · ${view.screen}` : ''}</span></p>
      ...
      {view.tab === 'overview' && <><Briefing companies={cohortCompanies} data={data} basis={view.basis} onScreen={screen} /><div className={s.sectionLink}><div><h2>From the market to a company</h2><p>Choose a sector below, or use the screener to build your own research list.</p></div><button className={s.primary} onClick={() => updateView({ tab: 'companies', screen: 'all', query: '' })}>Explore companies <ArrowUpRight size={16} /></button></div><SectorMap data={data} basis={view.basis} statistic={view.statistic} selectedCohort={view.cohort} onStatistic={(statistic) => updateView({ statistic })} onCohort={(cohort) => updateView({ cohort, tab: 'companies', query: '', screen: 'all' })} /><ObservationHistory data={data} /></>}
      {view.tab === 'sectors' && <SectorMap data={data} basis={view.basis} statistic={view.statistic} selectedCohort={view.cohort} onStatistic={(statistic) => updateView({ statistic })} onCohort={(cohort) => updateView({ cohort, tab: 'companies', query: '', screen: 'all' })} />}
