...
export type MarketIndustry = { code: string; label: string; known: boolean; count: number; metrics?: Record<string, Stats> };
export type MarketSector = { id: string; label: string; count: number; targetCount: number | null; industries: MarketIndustry[]; missingIndustryCount: number; metrics: Record<string, Stats> };
export type MarketSummary = { companyCount: number; sectorCount: number; industryCount: number; sectors: MarketSector[]; industries: MarketIndustry[]; missingSectorCount: number; missingIndustryCount: number; unknownIndustryCount: number; unknownIndustryCompanyCount: number; metrics: Record<string, Stats>; growth: Stats; profit: Stats; cash: Stats; olderReports: number; reportRange: { earliest: string; latest: string; count: number } | null; generatedAt: string | null; classificationSource: string };
export type MarketData = { viewVersion: string; themes: Cohort[]; historyNote?: string; coverage?: { membership_id: string; grouping?: string; membership_warning?: string; unsupported_issuers?: number; target_issuers: number; loaded_issuers: number; missing_issuers: number; source_securities: number; duplicate_share_classes: number; sources: { fund: string; as_of: string; url: string; securities: number }[] }; version: string; generatedAt: string; requested: number; companies: Company[]; cohorts: Cohort[]; failures: { ticker: string; reason: string }[]; historyPersistence: boolean; observations: { observedAt: string; companies: number; tickers: string[]; revenueGrowth: Stats; netMargin: Stats }[]; cache?: { status: 'stale'; warning: string } };
...
export type MarketBriefingData = { version: 'market-briefing-v1'; generatedAt: string; requested: number; coverage?: MarketData['coverage']; summaries: Record<Basis, MarketSummary>; failureCount: number; cache?: MarketData['cache']; historyNote?: string };
export type MarketDirectoryCompany = { ticker: string; name: string; cik: string; sector: string; revenueBasis?: string };
export type MarketDirectoryPage = { version: 'market-directory-v1'; generatedAt: string; query: string; sector: string; page: number; pageSize: number; total: number; companies: MarketDirectoryCompany[]; failureCount: number };
