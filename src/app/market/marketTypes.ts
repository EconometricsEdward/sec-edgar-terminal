...
export type MarketData = { version: string; generatedAt: string; requested: number; companies: Company[]; cohorts: Cohort[]; failures: { ticker: string; reason: string }[]; historyPersistence: boolean; observations: { observedAt: string; companies: number; tickers: string[]; revenueGrowth: Stats; netMargin: Stats }[]; cache?: { status: 'stale'; warning: string } };
