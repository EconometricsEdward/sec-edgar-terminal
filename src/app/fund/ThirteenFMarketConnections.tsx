...
export default function ThirteenFMarketConnections({ data, active, onInspectCompany }: Props) {
  ...
  return <div className={s.connections}>
    ...
    <div className={s.scan}>
      <div className={s.scanProgress}><div role="status" aria-live="polite">{remote.pending ? <RefreshCw size={13} className={s.spin} /> : remote.paused ? <Pause size={13} /> : <Check size={13} />}<span>{remote.pending ? "Reading SEC disclosures" : remote.paused ? "Review paused" : "SEC review"}<strong>{number(coverage.attempted)} of {number(coverage.total)} holdings checked</strong></span></div><progress value={Math.min(100, Math.max(0, remote.progress || 0))} max={100} aria-label="Progress through the selected holdings" /></div>
    ...
    <p className={s.scopeDates}>Reviewed in order of reported value · Company evidence: latest eligible annual filing and newer 10-Q · CFTC: latest available weekly positions. Company evidence and market observations may postdate the 13F snapshot.</p>
