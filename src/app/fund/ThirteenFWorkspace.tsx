...
import { ArrowDownToLine, ArrowRight, ArrowUpRight, Building2, Check, ChevronDown, ChevronLeft, ChevronRight, Clock3, FileText, Layers3, RefreshCw, Search, ShieldCheck, X } from "lucide-react";
...
function PositionName({ holding }: { holding: any }) {
  return <div className={s.positionName}><strong>{holding.issuer || "Issuer not supplied"}</strong><span>{holding.classTitle || "Class not supplied"}{holding.putCall ? <b className={s.option}>{holding.putCall}</b> : null}</span></div>;
}
...
export default function ThirteenFWorkspace({ settings, onPatch }: { settings: any; onPatch: (patch: any) => void }) {
  ...
  const view = ["overview", "holdings", "changes", "filings"].includes(settings.managerView) ? settings.managerView : "overview";
  ...
  function chooseView(managerView: string) { onPatch({ managerView }); }
