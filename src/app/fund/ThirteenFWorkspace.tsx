...
import { readCached13FReview } from "../../utils/thirteenFReviewClient.js";
...
/** Market publications open without awaiting or downloading a full manager
 * portfolio. The compact report supplies additional quarter choices in parallel. */
function MarketsWorkspace({ cik, period, onPatch, onCompare, onHistory }: { cik: string; period: string; onPatch: (patch: any) => void; onCompare: () => void; onHistory: (period: string, key: string) => void }) {
  const [attempt, setAttempt] = useState(0);
  const [saved, setSaved] = useState<any>(() => readCached13FReview(cik, period));
  const [panel, setPanel] = useState<{ data: any; holding: any } | null>(null);
  const compact = useReport(cik, period, true, attempt, SUMMARY_DELIVERY);
  const data = compact.data;
  const savedReport = saved?.job?.cik === cik && (!period || saved.job.period === period) ? saved.report : null;
  const selectedPeriod = savedReport?.period || period || data?.selectedPeriod || "";
  const managerName = savedReport?.managerName || data?.manager.name || `SEC manager ${cik}`;
  const matchingSummary = data?.selectedPeriod === selectedPeriod ? data : null;
  const reports: Report[] = [...(data?.reports || saved?.reports || [])];
  if (selectedPeriod && !reports.some(item => item.period === selectedPeriod)) reports.push({ period: selectedPeriod, filingCount: savedReport?.filings?.length || 1, latestFiled: "", forms: [] });
  reports.sort((a, b) => b.period.localeCompare(a.period));
  const report = reports.find(item => item.period === selectedPeriod);
  const complete = savedReport ? savedReport.complete : matchingSummary?.portfolio?.complete;
  const total = saved?.job?.total ?? matchingSummary?.portfolio?.positionCount;
  function inspect(holding: any, source?: any) {
    const origin = source || savedReport;
    const sourcePeriod = origin?.period || selectedPeriod;
    if (!sourcePeriod || origin?.cik && origin.cik !== cik || !holding?.key || !holding?.cusip || [holding.cusip, holding.putCall || "SECURITY", holding.quantityType].join("|") !== holding.key) return;
    // This exact row comes from the revision-bound shared API. Only that
    // revision's filing chain is attached; summary rows are never a substitute.
    const context = origin ? { manager: { cik, name: origin.managerName || managerName }, selectedPeriod: sourcePeriod, observedAt: origin.observedAt,
      portfolio: { holdings: [holding], totalValueUsd: origin.totalValueUsd, complete: origin.complete, filings: origin.filings || [] }, coverage: { selectedPeriodComplete: origin.complete } } : matchingSummary;
    if (context) setPanel({ data: context, holding });
  }
  function chooseManager(managerCik: string) { onPatch({ managerCik, managerPeriod: "", managerView: "overview" }); }
  return <div className={s.workspace}>
    <div className={s.managerTop}><button type="button" onClick={() => onPatch({ managerCik: "", managerPeriod: "", managerView: "overview" })}><ChevronLeft size={15} />Explore managers</button><button type="button" onClick={onCompare}><Network size={15} />Compare managers</button><Link href={`/filings/${cik}`}>All SEC filings<ArrowUpRight size={14} /></Link></div>
    <header className={s.managerHeader}><div><span className={s.eyebrow}><Building2 size={15} />Institutional holdings · Form 13F</span><h1>{managerName}</h1><div className={s.managerMeta}><span>CIK {cik}</span>{selectedPeriod ? <span>{savedReport ? "Saved review" : "Snapshot"} {date(selectedPeriod)}</span> : null}{report?.latestFiled ? <span>Latest filing {date(report.latestFiled)}</span> : null}</div></div><ManagerSearch compact onChoose={chooseManager} /></header>
    <div className={s.reportControls}><label>Reporting quarter<select aria-label="Reporting quarter" value={selectedPeriod} disabled={!reports.length} onChange={event => onPatch({ managerPeriod: event.target.value })}>{!reports.length ? <option value={selectedPeriod}>{selectedPeriod ? `${quarter(selectedPeriod)} · ${date(selectedPeriod)}` : "Finding available quarters…"}</option> : reports.map(item => <option key={item.period} value={item.period}>{quarter(item.period)} · {date(item.period)}{item.filingCount > 1 ? ` · ${item.filingCount} filings` : ""}</option>)}</select></label><div className={s.reportStatus}>{complete != null ? <span className={complete ? s.verified : s.partial}>{complete ? <Check size={14} /> : <Clock3 size={14} />}{complete ? "Public table reconciled" : "Coverage needs review"}</span> : null}<button type="button" disabled={compact.status === "loading"} onClick={() => setAttempt(value => value + 1)}><RefreshCw size={14} className={compact.status === "loading" ? s.spin : undefined} />{compact.status === "loading" ? "Checking quarters…" : "Check report quarters"}</button></div></div>
    {savedReport && data?.selectedPeriod && data.selectedPeriod !== savedReport.period && !period ? <p className={s.caption}>The saved market review covers {quarter(savedReport.period)}. A newer report, {quarter(data.selectedPeriod)}, is available in the quarter selector.</p> : null}
    {compact.status === "error" ? <p className={s.caption} role="status">Additional report quarters could not be checked. Saved market research remains available. <button type="button" onClick={() => setAttempt(value => value + 1)}>Retry quarter list</button></p> : null}
    <nav className={s.tabs} aria-label="13F research views">{[{ key: "overview", label: "Overview", icon: Layers3 }, { key: "holdings", label: "Holdings", icon: Building2 }, { key: "changes", label: "Quarterly changes", icon: RefreshCw }, { key: "history", label: "Portfolio history", icon: ChartNoAxesCombined }, { key: "markets", label: "Market connections", icon: Network }, { key: "filings", label: "Filings & evidence", icon: FileText }].map(tab => <button key={tab.key} type="button" aria-current={tab.key === "markets" ? "page" : undefined} onClick={() => onPatch({ managerView: tab.key, managerPeriod: selectedPeriod })}><tab.icon size={16} />{tab.label}{tab.key === "holdings" && finite(total) ? <span>{number(total)}</span> : null}</button>)}</nav>
    <ThirteenFMarketConnections cik={cik} period={period} data={matchingSummary} active={true} onSnapshot={setSaved} onInspectCompany={inspect} />
    {panel ? <ThirteenFCompanyPanel key={`${cik}:${panel.data.selectedPeriod}:${panel.holding.key}`} holding={panel.holding} data={panel.data} onClose={() => setPanel(null)} onViewHistory={(key: string) => onHistory(panel.data.selectedPeriod, key)} /> : null}
  </div>;
}
...
export default function ThirteenFWorkspace({ settings, onPatch }: { settings: any; onPatch: (patch: any) => void }) {
  ...
  const delivery = ["changes", "history"].includes(view) ? FULL_DELIVERY : view === "holdings" ? HOLDINGS_DELIVERY : SUMMARY_DELIVERY;
  const current = useReport(cik, period, !!cik && view !== "compare" && view !== "markets", attempt, delivery);
  ...
  if (view === "markets") return <MarketsWorkspace key={`${cik}:${period}`} cik={cik} period={period} onPatch={onPatch} onCompare={compareManagers} onHistory={(sourcePeriod, key) => { setHistoryHolding({ cik, period: sourcePeriod, key }); onPatch({ managerView: "history", managerPeriod: sourcePeriod }); }} />;
  const portfolio = data?.portfolio;
