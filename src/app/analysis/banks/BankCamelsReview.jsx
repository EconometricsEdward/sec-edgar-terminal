...
export default function BankCamelsReview({data,period,onExplore,history,historyPanel,onHistory}) {
  const [display,setDisplay]=useState('history'),[baseline,setBaseline]=useState('prior');
  ...
  return <section className={styles.camelsReview} aria-label="CAMELS public-data review">
    ...
    <div className={styles.camelsHistoryControls}><div><strong>{display==='history'?'How the bank is changing':'Position within the peer group'}</strong><p>{display==='history'?`${history?.periods.length?`${quarterLabel(history.periods[0])} → ${quarterLabel(period)} · `:''}Bank trend lines use a separate scale for each metric. Select one to explore.`:'Relative numeric rank for the selected quarter.'}</p></div><div className={styles.historyControlsRight}><div className={styles.historyToggle} role="group" aria-label="CAMELS metric display"><button aria-pressed={display==='history'} onClick={()=>setDisplay('history')}>Over time</button><button aria-pressed={display==='position'} onClick={()=>setDisplay('position')}>Peer position</button></div>{display==='history'&&<label>Changes vs <select aria-label="CAMELS change baseline" value={baseline} onChange={e=>setBaseline(e.target.value)}><option value="prior">Prior quarter</option><option value="first">First available period</option></select></label>}</div></div>
    <div className={styles.camelsGrid} data-display={display}>{CAMELS_DIMENSIONS.map(d=><article className={styles.camelsCard} key={d.key} style={{'--accent':d.color}}>
      ...
      <dl>{d.keys.map(key=>{const m=metrics.get(key),notRequired=data.bank?.cblr===true&&m?.riskBased;return <div className={styles.camelsMetric} key={key}>
        ...
        {display==='history'?<MetricHistoryMini metric={trends.get(key)} baseline={baseline} onOpen={()=>onHistory(key)}/>:m?.percentile!=null&&!notRequired&&<div className={styles.percentileLine} role="img" aria-label={`${m.label}: percentile ${Math.round(m.percentile)} of ${m.count} reported peers. Numeric rank, not quality.`}><span style={{width:`${m.percentile}%`}}/><i style={{left:`${m.percentile}%`}}/><small>P{Math.round(m.percentile)}</small></div>}
      </div>;})}</dl><p>{d.note}</p><button onClick={()=>onExplore(d.key)}>Explore {d.shortLabel?.toLowerCase()||d.label.toLowerCase()} →</button>
    </article>)}</div>
    <p className={styles.basis}>{display==='history'?'Changes are in basis points (100 bp = 1 percentage point), not percentage growth. Direction is not a quality score.':'P0–P100 shows relative numeric position, not “better” or “worse.”'} Each metric uses its own valid peer count. CBLR banks are excluded from risk-based capital distributions.</p>
    {historyPanel}
