...
export default function BankPeerHistory({history,error,onRetry,metricKey,onMetricChange,period,sectionRef,bankName}){
  ...
  const bankStatus=points.length&&!points.some(p=>typeof p.value==='number'&&Number.isFinite(p.value))?(points.every(p=>p.notRequired)?' · N/A · CBLR':' · Unavailable'):last?.notRequired?' · Current period N/A · CBLR':last?.value==null?' · Current period unavailable':'';
  return <section ref={sectionRef} tabIndex={-1} className={styles.peerHistory} aria-label="Metric history">
    ...
    {points.length>0&&<>
      <div className={styles.historyLegend}><span><i className={styles.bankLine}/>{bankName}{bankStatus}</span><span><i className={styles.dashedSwatch}/>Peer-group median · {last.peerCount} / {history.cohort.length} reporting now</span><small>Ratios in % · Changes in bp</small></div>
