...
export function MetricHistoryMini({metric,onOpen,baseline='prior',bankName='Selected bank'}){
  if(!metric?.points?.length)return <button className={styles.metricMini} onClick={onOpen}><span className={styles.positionUnavailable}>View history ↗</span></button>;
  ...
  const peerDelta=from===last.period?null:peerChange(points,from,'peerMedian');
  const values=points.flatMap(p=>[p.value,p.peerMedian]).filter(finite),min=values.length?Math.min(...values):0,max=values.length?Math.max(...values):0;
  ...
  return <button type="button" className={styles.metricMini} onClick={onOpen} aria-label={`Explore ${metric.label} history. ${bankName}: ${formatBasisPoints(change)}. Peer-group median: ${formatBasisPoints(peerDelta)}. Changes versus ${quarterLabel(from)}.`}>
    <svg viewBox="0 0 128 38" aria-hidden="true">
      ...
      {['peerMedian','value'].map(key=>points.map((p,i)=>finite(p[key])&&<g key={`${key}-${p.period}`} style={{color:key==='value'?'var(--bank-color)':'var(--peer-color)'}}>
        {i>0&&finite(points[i-1][key])&&<line x1={x(i-1)} y1={y(points[i-1][key])} x2={x(i)} y2={y(p[key])} stroke="currentColor" strokeWidth="2" strokeDasharray={key==='peerMedian'?'4 3':undefined}/>}
        <circle cx={x(i)} cy={y(p[key])} r={i===points.length-1?3:2} fill="currentColor"/>
      </g>))}
      {!values.length&&<text x="64" y="22" textAnchor="middle" fill="#9fb3cd" fontSize="10">No reported values</text>}
    </svg>
    <span className={styles.miniChanges}><span className={styles.miniBank}>Bank <b>{change==null?'—':formatBasisPoints(change)}</b></span><span className={styles.miniPeer}>Median <b>{peerDelta==null?'—':formatBasisPoints(peerDelta)}</b></span><small>{points.length===1?'One period only':`${baseline==='first'?'since':'vs'} ${quarterLabel(from)}`} ↗</small></span>
  </button>;
}
...
export default function BankPeerHistory({history,error,onRetry,metricKey,onMetricChange,period,sectionRef,bankName}){
  ...
  return <section ref={sectionRef} tabIndex={-1} className={styles.peerHistory} aria-label="Metric history">
    ...
    {points.length>0&&<>
      ...
      <div className={styles.historyLegend}><span><i className={styles.bankLine}/>{bankName}</span><span><i className={styles.dashedSwatch}/>Peer-group median · up to {history.cohort.length} banks</span><small>Ratios in % · Changes in bp</small></div>
