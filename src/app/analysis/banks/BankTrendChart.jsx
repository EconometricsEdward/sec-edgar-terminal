...
export default function BankTrendChart({ points, quarterly, unit, label }) {
  ...
  return <div style={{ width: '100%', height: 290 }} role="img" aria-label={`${label} across reporting periods. Exact figures are in the table below.`}>
    <ResponsiveContainer width="100%" height="100%"><Chart data={points} margin={{ top: 20, right: 22, left: 14, bottom: 10 }} accessibilityLayer>
      ...
      <YAxis stroke="#a5b6cb" width={78} tick={{ fontSize: 11 }} tickLine={false} axisLine={false} tickFormatter={v => new Intl.NumberFormat('en-US', { notation: 'compact', maximumFractionDigits: 1 }).format(v)} domain={quarterly ? ['auto', 'auto'] : ['auto', 'auto']} />
