...
export default function BankPeerBenchmarks({rssd,period,onCompare,lens='peers',category='core',onViewChange}){
  ...
  return <div className={styles.benchmarks} ref={wrapperRef}>
    ...
    {bank&&<section className={styles.peerContext} ref={contextRef} aria-label="Selected bank and peer group">
      ...
      <div className={styles.contextPeers}><span className={styles.contextScope}><i className={styles.peerDiamond}/>Peer group · aggregate values</span><strong>{peers.length} matched banks <button onClick={exploreGroup}>View group ↗</button></strong><small>Unweighted medians · Bank excluded{data.assetBand===8?' · Expanded size range':''}</small></div>
    </section>}
    {lens==='camels'&&bank?<BankCamelsReview data={data} period={period} history={history} onHistory={exploreHistory} category={category} onCategory={category=>onViewChange({category})}/>:!bank?<section className={styles.empty}><h3>No matching quarterly FDIC record</h3><p>Choose another quarter or compare selected banks.</p><button onClick={()=>onCompare([])}>Open selected-bank comparison</button></section>:<BankPeerExplorer data={data} history={history} metricKey={historyMetric} onMetricChange={setHistoryMetric} onHistory={exploreHistory} onCompare={onCompare}/>}
    {bank&&<details className={styles.historyDrawer} ref={historyDetailsRef} tabIndex={-1} open={historyOpen} onToggle={e=>setHistoryOpen(e.currentTarget.open)}><summary>Metric history <span>{data.benchmarks.find(m=>m.key===historyMetric)?.label} · Available periods</span></summary>{historyOpen&&historyPanel}</details>}
