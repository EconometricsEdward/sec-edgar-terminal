...
export default function BankWorkspace({ initialState, rssd, options, error }) {
  ...
  return <div className={styles.page}>
    ...
    {options.view === 'compare' && <>
      <div className={styles.peerChips}>{banks.map((b, i) => <span key={b.id_rssd}><strong>{b.legal_name}</strong><small>RSSD {b.id_rssd}{i === 0 ? ' · Selected bank' : ''}</small>{i > 0 && <button type="button" onClick={() => navigate({ peers: options.peers.filter(id => !sameBank(id, b.id_rssd)) })} aria-label={`Remove ${b.legal_name}`}>×</button>}</span>)}</div>
