...
export default function BankWorkspace({ initialState, rssd, options: initialOptions, error }) {
  ...
  return <div className={`${styles.page} ${['compare','exposures','organization'].includes(options.view)?styles.comparePage:''}`} aria-busy={navigating || !selectionReady}>
    ...
    {options.view !== 'organization' && (options.view !== 'compare' || options.panel === 'selected') && <Preparation bank={bank} state={state} requesting={requesting} onPrepare={prepare} />}
    ...
    {options.view === 'organization' && <BankOrganization rssd={rssd} bankName={bank?.legal_name || `RSSD ${rssd}`} options={options} href={href} onChange={navigate} />}
