...
type Measure = { id: string; label: string; value: number | null; unit?: string; format?: string; periodEnd?: string; periodStart?: string; durationMonths?: number; source?: Source; sources?: Source[]; formula?: string; confidence?: string; basis?: string; statement?: string; section?: string };
type Filing = { accession?: string; accessionNumber?: string; reportDate?: string; filingDate?: string; form?: string; archive?: string; archiveFile?: string; researchHref?: string; analysisHref?: string; periodKey?: string };
type Research = { company?: { name?: string; cik?: string } | null; name?: string; filing?: Filing; analysis?: any; brokerDealerAnalysis?: any; selectedDocument?: { url?: string; name?: string }; documents?: Array<{ name: string; url: string; description?: string }>; extraction?: any; observedAt?: string };
...
function latestPeriods(filings: Filing[], limit = 5) {
  ...
  return ordered(filings).filter(row => { const key = row.reportDate || accessionOf(row); if (seen.has(key)) return false; seen.add(key); return true; }).slice(0, limit);
}
...
export default function BrokerDealerWorkspace({ cik, initialResearch, discovery, explicitSelection = false }: { cik: string; initialResearch?: Research | null; discovery: any; explicitSelection?: boolean }) {
  ...
  const selectedFilings = useMemo(() => ordered(catalog.filter(row => selected.includes(accessionOf(row))).map(row => reports[accessionOf(row)]?.filing || row)), [catalog, reports, selected]);
  ...
  const chronological = latestPeriods(selectedFilings.filter(row => row.reportDate), 10).reverse();
  ...
  const limits = [...new Set<string>([...(analysis.limitations || []), ...(analysis.coverage?.caveats || [])])];

  function chooseActive(key: string) { customized.current = true; setActive(key); if (!selected.includes(key)) setSelected(previous => [...previous.slice(0, 9), key]); }
  ...
  function applyPeriods() { if (!draft.length) return; customized.current = true; setSelected(draft); if (!draft.includes(active)) setActive(ordered(catalog.filter(row => draft.includes(accessionOf(row))))[0] ? accessionOf(ordered(catalog.filter(row => draft.includes(accessionOf(row))))[0]) : draft[0]); setPickerOpen(false); }
  ...
  return <section className={styles.workspace} aria-label="Broker-dealer financial research">
    <div className={styles.controlBar}><div className={styles.periodControl}><span className={styles.overline}>Report period</span><label><span className={styles.srOnly}>Select active report period</span><select value={active} onChange={event => chooseActive(event.target.value)}>{catalog.map(row => <option key={accessionOf(row)} value={accessionOf(row)}>{periodOf(reports[accessionOf(row)]?.filing || row)} · {row.form || "X-17A-5"} · Filed {row.filingDate}</option>)}</select></label></div><div className={styles.controlActions}><span className={styles.loadStatus} role="status">{loading ? <LoaderCircle size={13} className={styles.spinner} /> : <Check size={13} />} {loaded} of {selected.length} reports read</span><button type="button" className={styles.periodButton} onClick={() => { setDraft(selected); setPickerOpen(!pickerOpen); }} aria-expanded={pickerOpen}><SlidersHorizontal size={15} /> Adjust periods <ChevronDown size={13} /></button></div></div>
    {pickerOpen && <div className={styles.periodPicker}><div className={styles.panelHeading}><div><h3>Choose reports to compare</h3><p>The five most recent periods are selected by default. Choose up to ten reports; amendments retain their own filing date.</p></div><button type="button" aria-label="Close period selection" onClick={() => setPickerOpen(false)}><X size={16} /></button></div><div className={styles.periodChoices}>{catalog.map(row => { const key = accessionOf(row); return <label key={key} className={draft.includes(key) ? styles.checkedPeriod : undefined}><input type="checkbox" checked={draft.includes(key)} disabled={!draft.includes(key) && draft.length >= 10} onChange={() => setDraft(previous => previous.includes(key) ? previous.filter(id => id !== key) : [...previous, key])} /><span><strong>{periodOf(reports[key]?.filing || row)}</strong><small>{row.form} · Filed {row.filingDate}</small></span></label>; })}</div><div className={styles.pickerActions}><button type="button" onClick={() => setDraft(latestPeriods(catalog).map(accessionOf))}>Latest five periods</button>{archives.some(item => !item.loaded && !item.checked) && <button type="button" onClick={loadOlder} disabled={archiveLoading}>{archiveLoading ? "Checking history…" : "Check older SEC history"}</button>}<button type="button" className={styles.applyButton} disabled={!draft.length} onClick={applyPeriods}>Apply {draft.length} reports</button></div></div>}
    {historyError && <p className={styles.inlineNotice}>{historyError} The reports already available remain usable.</p>}
    <div className={styles.periodRibbon} aria-label="Selected report periods">{selectedFilings.map(row => { const key = accessionOf(row); return <button key={key} type="button" aria-pressed={key === active} onClick={() => setActive(key)} className={key === active ? styles.activePeriod : undefined}><span className={`${styles.periodDot} ${reports[key] ? styles.readyDot : errors[key] ? styles.errorDot : ""}`} /><span>{periodOf(row)}{row.form?.endsWith("/A") ? " · amended" : ""}</span>{!reports[key] && !errors[key] && <span className={styles.srOnly}>Loading</span>}</button>; })}</div>
    ...
    {!current && <div className={styles.loadingPanel} role="status">{errors[active] ? <><FileText size={25} /><h3>This report is temporarily unavailable</h3><p>{errors[active]}</p><button type="button" onClick={retryReport} disabled={busy[active]}>Try this report again</button></> : <><LoaderCircle size={25} className={styles.spinner} /><h3>Reading this annual report</h3><p>The other selected reports load in the background. Scanned PDFs can take longer to read.</p></>}</div>}
    ...
    {section === "overview" && current && <>
      ...
      <TrendChart periods={chartPeriods} {...groups.balance} />
    ...
    {section === "statements" && <><div className={styles.sectionHeading}><div><span className={styles.overline}>Statements</span><h2>Compare the disclosed financials</h2><p>Exact USD amounts with evidence from each report.</p></div></div><div className={styles.statementTabs} aria-label="Select financial statement">{STATEMENTS.map(item => <button key={item.id} type="button" aria-pressed={statement === item.id} className={statement === item.id ? styles.activePill : undefined} onClick={() => setStatement(item.id)}>{item.label}</button>)}</div><StatementTable reports={reports} filings={selectedFilings} statement={statement} active={active} errors={errors} /></>}
    {section === "trends" && <><div className={styles.sectionHeading}><div><span className={styles.overline}>Across reporting periods</span><h2>See how the business changes</h2><p>Hover or focus a date for exact values. Missing periods remain visible as gaps.</p></div></div><div className={styles.statementTabs} aria-label="Select trend measures">{[{ id: "balance", label: "Balance sheet" }, { id: "capital", label: "Capital" }, { id: "earnings", label: "Earnings" }, { id: "cash", label: "Cash flow" }, { id: "leverage", label: "Leverage" }].map(item => <button key={item.id} type="button" aria-pressed={chartGroup === item.id} className={chartGroup === item.id ? styles.activePill : undefined} onClick={() => setChartGroup(item.id)}>{item.label}</button>)}</div><TrendChart periods={chartPeriods} {...activeGroup} />{suppressedFlows && ["earnings", "cash"].includes(chartGroup) && <p className={styles.inlineNotice}>Some flow measures have different or unconfirmed reporting durations. Their trend lines are withheld; the Statements view retains reported amounts with duration labels.</p>}<p className={styles.visualNote}>When multiple versions of one period are selected, charts use its latest selected filing. Both versions remain available in Statements.</p><div className={styles.twoColumn}><TrendChart periods={chartPeriods} {...groups.capital} /><TrendChart periods={chartPeriods} {...groups.leverage} /></div></>}
    {section === "ratios" && <><div className={styles.sectionHeading}><div><span className={styles.overline}>Financial profile</span><h2>Leverage, liquidity & capital</h2><p>Calculated from compatible reported inputs. Expand any calculation to inspect its source.</p></div></div><div className={styles.ratioGrid}>{["assetsToEquity", "equityToAssets", "cashToAssets", "netCapitalToRequired", "reverseReposToAssets", "reposToLiabilities"].map(id => { const metric = metricMap[id]; return metric ? <article key={id}><span>{metric.label}</span><strong>{displayAmount(metric.value, valueFormat(metric))}</strong><p>{metric.formula}</p><Evidence measure={metric} /></article> : null; })}</div><StatementTable reports={reports} filings={selectedFilings} ratios active={active} errors={errors} /><p className={styles.visualNote}>Ratios describe the disclosed financial structure. Accounting leverage is not collateral-adjusted; regulatory capital differs from equity.</p></>}
