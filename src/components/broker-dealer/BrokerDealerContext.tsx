...
import { filerCik, hasBrokerDealerFilings } from '../../utils/secFilerSearch.js';
...
import { BROKER_DEALER_PEER_CANDIDATES, BROKER_PEER_COLUMNS, brokerPeerRow, brokerCftcSeries, compareBrokerPeriods, fetchBrokerPeer, brokerComparisonFit, brokerReportIdentity } from '../../utils/brokerDealerContext.js';
...
function Peers({ cik, research }: Props) {
  ...
  const matches = filerSearch.results.filter(hasBrokerDealerFilings);
  const rows = [baseline, ...peers.filter(peer => peer.status === 'ready').map(peer => brokerPeerRow(peer.data, peer.cik))].filter(Boolean).map(row => {
    const fit = row!.cik === cik ? { compatible: true, label: 'Selected report' } : brokerComparisonFit(baseline, row);
    return { ...row!, identity: brokerReportIdentity(row), fit, measures: fit.compatible ? row!.measures : {} };
  });
  ...
  async function addPeer(candidate: { cik: string; name: string }) {
    ...
    try {
      ...
      if (!controller.signal.aborted) setPeers(current => current.map(peer => peer.cik === candidate.cik ? { ...candidate, name: data.company.name, status: 'ready', data } : peer));
    } catch (error: any) {
      if (!controller.signal.aborted) setPeers(current => current.map(peer => peer.cik === candidate.cik ? { ...candidate, status: 'error', error: error?.message || 'The public report could not be read. Retry this peer.' } : peer));
  ...
  return <>
    <p className={styles.intro}>Compare the selected filing with public X-17A-5 reports from up to three other SEC registrants. Report type, FOCUS part and reporting scope are checked before measures enter the comparison.</p>
    ...
    {rows.length > 1 ? <div className={styles.comparison}>
      <div className={styles.barPanel}><div className={styles.chartTitle}><h3>Across the selected entities</h3><label>Compare measure<select value={metricId} onChange={event => setMetricId(event.target.value)}>{BROKER_PEER_COLUMNS.map(column => <option key={column.id} value={column.id}>{column.label}</option>)}</select></label></div><div className={styles.bars}>{rows.map(row => <div key={row!.cik} className={styles.barRow}><div><a href={row!.url}>{row!.name}</a><span>{row!.periodEnd}</span></div><div className={styles.barTrack} aria-hidden="true"><i data-primary={row!.cik === cik} style={{ width: `${largest && finite(row!.measures[metricId]?.value) ? Math.abs(row!.measures[metricId].value) / largest * 100 : 0}%` }} /></div><strong>{row.fit.compatible ? display(row!.measures[metricId]?.value, metric.format) : "Not comparable"}</strong></div>)}</div><p className={styles.note}>Bar lengths show magnitude; labels retain the sign. Reporting dates remain visible. This is a comparison of reported measures, without peer ranks or a composite risk score.</p></div>
      <div className={styles.tableWrap}><table><caption>Source-linked broker-dealer comparison</caption><thead><tr><th scope="col">SEC registrant / period</th>{BROKER_PEER_COLUMNS.map(column => <th scope="col" key={column.id}>{column.label}</th>)}</tr></thead><tbody>{rows.map(row => { const dateFit = compareBrokerPeriods(baseline?.periodEnd, row!.periodEnd); return <tr key={row!.cik}><th scope="row"><a href={row!.url}>{row!.name}</a><small>CIK {row!.cik} · {row!.periodEnd}</small><small>{row.identity.label}{row.identity.parts.length ? ` · ${row.identity.parts.join(" / ")}` : ""}</small><small>{row.identity.auditLabel}{row.identity.auditScope ? ` · ${row.identity.auditScope}` : ""}</small>{!row.fit.compatible && <small className={styles.periodMismatch}>{row.fit.label}</small>}<small className={!dateFit.aligned ? styles.periodMismatch : undefined}>{row!.cik === cik ? 'Selected report' : dateFit.label}{row!.status === 'partial' ? ' · Partial coverage' : ''}</small></th>{BROKER_PEER_COLUMNS.map(column => { const amount = row!.measures[column.id]; return <td key={column.id}>{amount ? <a href={amount.sourceUrl} target="_blank" rel="noreferrer" title={`${amount.label}: ${amount.value.toLocaleString('en-US')} · ${row!.periodEnd} · source page ${amount.source?.page || ''}`}>{display(amount.value, column.format)}<ArrowUpRight size={11} /></a> : <span className={styles.missing}>{row.fit.compatible ? "Not disclosed" : "Not comparable"}</span>}</td>; })}</tr>; })}</tbody></table></div>
      <p className={styles.note}>Each peer uses its latest available public X-17A-5 report; the selected company uses your current report. Annual and periodic reports, different FOCUS parts, and unclassified documents are not combined. Incompatible measures are withheld; open a peer to inspect its report individually. Dates remain unaligned and amounts are not annualized. Accounting equity, net capital and required minimums measure different things. Missing amounts remain unavailable.</p>
