...
function query(request) {
  ...
  if ([...params.keys()].some(key => !['cik', 'period', 'reportHash', 'market', 'status', 'query', 'offset', 'limit', 'key'].includes(key) || params.getAll(key).length !== 1))
...
export function createThirteenFReviewApi({ store = thirteenFReviewStore, portfolioLoader = loadThirteenF,
  rateLimit = checkRateLimit, enabled = isCftcEnabled, schedule = () => {} } = {}) {
  ...
  return {
    async GET(request) {
      try {
        ...
        const data = params.key ? await store.result(params, options) : await store.read(params, options);
        ...
        return Response.json(data || { job: null }, { headers: data?.job || params.key ? PUBLIC : PRIVATE });
