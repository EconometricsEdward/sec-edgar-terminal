export function normalizeReportRequest(params) {
  ...
  if (kind === 'market') {
    if (id !== 'MARKET' || !['annual', 'ttm'].includes(basis)) {
      throw Object.assign(new Error('Choose the market report with annual or trailing-twelve-month fundamentals.'), { status: 400 });
    }
    return { kind, id, basis };
  }
...
export function reportMatchesSelection(report, { kind, id, basis }) {
  return report?.schema === 'edgar.report.v1' && report.kind === kind
    ...
    && (!['company', 'market'].includes(kind) || report.period?.basis === basis)
    && typeof report.entity.name === 'string' && report.entity.name.length > 0
    && (kind === 'market' ? id === 'MARKET' && report.entity.cik === '' : /^\d{1,10}$/.test(report.entity.cik || ''))
