...
export type ReportDocument = {
  ...
  period: { label: string; asOf: string | null; filingDate: string | null; basis?: string; start?: string | null; frequency?: string };
  ...
  classification?: { version: number; family: 'annual-report' | 'periodic-focus' | 'unknown'; label: string; parts: string[];
    audit: { status: 'auditor-report-present' | 'explicitly-unaudited' | 'not-established'; scope?: string | null };
    period: { start: string | null; end: string | null; frequency: string }; components: string[];
    evidence: { kind: string; page: number; url: string; excerpt: string; sourceId?: string }[]; limitations: string[] };
...
export type ReportSearchResult = { kind: ReportKind; id: string; name: string; ticker?: string; cik: string; seriesId?: string; brokerDealerForm?: 'X-17A-5'; detail: string };
