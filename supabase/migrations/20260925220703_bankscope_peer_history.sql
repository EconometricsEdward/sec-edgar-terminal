-- Read a fixed selected-quarter cohort across at most four retained quarters.
-- Only completed v2 snapshots; selected quarter is pinned to the matching snapshot.
-- No source downloads, new tables, broader grants, or changes to ingestion fences.
...
declare c edgar_private.bank_pilot_control%rowtype; s edgar_private.bank_peer_snapshots%rowtype;
  ...
  t timestamptz:=clock_timestamp(); requested_owner uuid; result jsonb; added integer; inserted integer; target_date date; target_id bigint; selected_ids bigint[];
begin
  ...
  if p_operation='peer_status' then
    return jsonb_build_object('snapshots',(select coalesce(jsonb_agg(to_jsonb(x)),'[]') from (
      select distinct on(report_date) report_date,completed_at,matched_count,source_index,source_updated_at,model_version from edgar_private.bank_peer_snapshots
  ...
  elsif p_operation='peer_history' then
    if coalesce(p_payload->>'rssd','') !~ '^[1-9][0-9]{0,9}$'
      or coalesce(p_payload->>'period','') !~ '^[0-9]{4}-(03-31|06-30|09-30|12-31)$'
      or jsonb_typeof(p_payload->'peers') is distinct from 'array'
      or exists(select 1 from jsonb_object_keys(p_payload) k where k not in ('rssd','period','peers','snapshotId')) then
      raise exception 'invalid_peer_history';
    end if;
    if jsonb_array_length(p_payload->'peers')>30 or exists(select 1 from jsonb_array_elements_text(p_payload->'peers') x
      where coalesce(x,'') !~ '^[1-9][0-9]{0,9}$') then raise exception 'invalid_peer_history'; end if;
    target_date:=(p_payload->>'period')::date; target_id:=(p_payload->>'rssd')::bigint;
    select coalesce(array_agg(x::bigint),'{}'::bigint[]) into selected_ids from jsonb_array_elements_text(p_payload->'peers') x;
    if target_id=any(selected_ids) or cardinality(selected_ids)<>(select count(distinct x) from unnest(selected_ids) x) then raise exception 'invalid_peer_history'; end if;
    select * into s from edgar_private.bank_peer_snapshots where id=(p_payload->>'snapshotId')::uuid
      and report_date=target_date and completed_at is not null and model_version='bankscope-peers-2';
    if not found then raise exception 'invalid_history_snapshot'; end if;
    with dates as materialized (
      select d::date as report_date from edgar_private.bank_pilot_control pc,
        jsonb_array_elements_text(pc.catalog->'periods') d
      where pc.id and d::date<=target_date order by d::date desc limit 4
    ), versions as materialized (
      select v.* from dates d cross join lateral (
        select z.* from edgar_private.bank_peer_snapshots z where z.report_date=d.report_date
          and z.completed_at is not null and z.model_version='bankscope-peers-2'
          and (d.report_date<>target_date or z.id=s.id)
        order by z.completed_at desc,z.id limit 1
      ) v
    ) select jsonb_build_object(
      'periods',(select coalesce(jsonb_agg(report_date order by report_date),'[]') from dates),
      'snapshots',(select coalesce(jsonb_agg(to_jsonb(v)-'owner' order by v.report_date),'[]') from versions v),
      'profiles',(select coalesce(jsonb_agg(jsonb_build_object('period',v.report_date,'rssd',p.id_rssd,
        'cblr',p.profile->'cblr','metrics',p.profile->'metrics')),'[]') from versions v
        join edgar_private.bank_peer_profiles p on p.snapshot_id=v.id
        and p.id_rssd=any(array_prepend(target_id,selected_ids)))
    ) into result;
    return result;
