-- Complete quarterly snapshots are published atomically. Source records stay private.
create table edgar_private.bank_peer_snapshots (
  id uuid primary key, report_date date not null, owner uuid not null, created_at timestamptz not null default now(),
  completed_at timestamptz, expected_count integer not null check(expected_count between 1000 and 10000),
  received_count integer not null default 0, matched_count integer not null default 0,
  source_url text not null, source_sha256 text not null check(source_sha256 ~ '^[a-f0-9]{64}$'),
  source_index text not null, source_updated_at timestamptz, model_version text not null
);
create index bank_peer_snapshots_period on edgar_private.bank_peer_snapshots(report_date,completed_at desc) where completed_at is not null;
create table edgar_private.bank_peer_profiles (
  snapshot_id uuid not null references edgar_private.bank_peer_snapshots(id),
  id_rssd bigint not null references edgar_private.bank_institutions(id_rssd), profile jsonb not null, raw_source jsonb not null,
  primary key(snapshot_id,id_rssd)
);
create table edgar_private.bank_ubpr_jobs (
  id_rssd bigint not null references edgar_private.bank_institutions(id_rssd), report_date date not null,
  status text not null default 'queued' check(status in ('queued','running','retry','ready','unavailable','review')),
  ...
  primary key(id_rssd,report_date)
);
create index bank_ubpr_jobs_pending on edgar_private.bank_ubpr_jobs(next_attempt_at,report_date desc) where status in ('queued','retry');
create table edgar_private.bank_ubpr_reports (
  id_rssd bigint not null references edgar_private.bank_institutions(id_rssd), report_date date not null,
  source_sha256 text not null check(source_sha256 ~ '^[a-f0-9]{64}$'), raw_xbrl text not null,
  retrieved_at timestamptz not null, data jsonb not null default '{}', primary key(id_rssd,report_date,source_sha256)
);
alter table edgar_private.bank_peer_snapshots enable row level security;
alter table edgar_private.bank_peer_profiles enable row level security;
alter table edgar_private.bank_ubpr_jobs enable row level security;
alter table edgar_private.bank_ubpr_reports enable row level security;
revoke all on edgar_private.bank_peer_snapshots,edgar_private.bank_peer_profiles,edgar_private.bank_ubpr_jobs,edgar_private.bank_ubpr_reports from public,anon,authenticated;
grant select,insert,update,delete on edgar_private.bank_peer_snapshots,edgar_private.bank_peer_profiles,edgar_private.bank_ubpr_jobs,edgar_private.bank_ubpr_reports to service_role;
-- Backfill official reports for banks already requested, within the existing fenced worker.
insert into edgar_private.bank_ubpr_jobs(id_rssd,report_date)
  select distinct p.id_rssd,p.report_date from edgar_private.bank_panel_entries p join edgar_private.bank_institutions b using(id_rssd)
  where b.last_requested_at>now()-interval '30 days' and p.has_filed;

alter function public.bank_scope_operation(text,jsonb) rename to bank_scope_call_operation;
revoke all on function public.bank_scope_call_operation(text,jsonb) from public,anon,authenticated;
grant execute on function public.bank_scope_call_operation(text,jsonb) to service_role;
